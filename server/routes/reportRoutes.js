...
// Removed: import Journal from '../models/Journal.js';
...
const getAggregatedReportData = async (userId, queryStartDate, queryEndDate) => {
    ...
    // Removed: Wellness Statistics (from Journal)
...
router.get('/export/:format', auth, asyncHandler(async (req, res) => {
    ...
    // Removed: Wellness Statistics section in PDF generation
    ...
    if (reportData.glucoseStats.readings.length > 0) {
        ...
        yPos -= lineHeight + 5;
        // Adjust loop for available space and pagination
        const maxReadingsPerPage = Math.floor((yPos - margin) / lineHeight);
        const readingsToDisplay = reportData.glucoseStats.readings.slice(0, maxReadingsPerPage > 0 ? maxReadingsPerPage : 10); // Display up to maxReadingsPerPage, or 10 if calculation is bad

        readingsToDisplay.forEach(reading => {
            // This basic check will only create one additional page.
            // For a robust solution, you'd need a more advanced page break calculation loop.
            if (yPos < margin) {
                page = pdfDoc.addPage();
                yPos = height - margin;
